...
@ Weather "continues" / overworld-weather animations. The weather tint is held
@ for the whole battle by src/battle_weather_fx.c, so these no longer fade the
@ palettes in and out (that made the tint blink every turn).
General_Rain:
	loadspritegfx ANIM_TAG_RAIN_DROPS
	playsewithpan SE_M_RAIN_DANCE, SOUND_PAN_ATTACKER
...
General_Sun:
	loadspritegfx ANIM_TAG_SUNLIGHT
	monbg ANIM_ATK_PARTNER
	setalpha 11, 5 @ 15% less bright than Sunny Day (v1.4)
	panse_adjustnone SE_M_PETAL_DANCE, SOUND_PAN_ATTACKER, SOUND_PAN_TARGET, +1, 0
	call SunnyDayLightRay
	call SunnyDayLightRay
	call SunnyDayLightRay
