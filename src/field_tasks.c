...
static void RunTimeBasedEvents(s16 *data)
{
    ...
    case 1:
        if (!(gMain.vblankCounter1 & TIME_UPDATE_INTERVAL))
        {
            // v1.6: the dynamic-weather check runs half a cycle after the
            // game's own time events, so their clock reads (interrupts off on
            // real hardware) never pile up in the same frame.
            TryRefreshDynamicWeather();
            UpdateRainPuddles();
            tState--;
        }
        break;
...
void SetUpFieldTasks(void)
{
    ...
    if (!FuncIsActiveTask(Task_OverworldWildEncounters))
        CreateTask(Task_OverworldWildEncounters, 80);
}
