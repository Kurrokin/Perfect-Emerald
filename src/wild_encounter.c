...
bool8 StandardWildEncounter(u16 curMetatileBehavior, u16 prevMetatileBehavior)
{
    ...
    else
    {
        ...
        {
            if (OweSuppressRandomGrassEncounter(curMetatileBehavior))
                return FALSE;   // OWE on: grass Pokemon are out walking instead
...
// ---- v3.0b overworld wild encounters (OWE) ----------------------------------
bool8 OweChooseLandMon(u16 *species, u8 *level)
{
    ...
    *species = info->wildPokemon[index].species;
    *level = ChooseWildMonLevel(&info->wildPokemon[index]);
    return TRUE;
}
