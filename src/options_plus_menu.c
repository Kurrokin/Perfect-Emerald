...
} static const sItemFunctionsCustom[MENUITEM_BATTLE_COUNT] =
{
    ...
    [MENUITEM_BATTLE_BACKGROUND]       = {DrawChoices_New_Backgrounds,    ProcessInput_Options_Three},
...
static const u8 sText_Desc_NewBackgrounds_Cfru[]   = _("Perfect Emerald backgrounds:\nCFRU with new grass and more.");
...
static const u8 *const sOptionMenuItemDescriptionsCustom[MENUITEM_BATTLE_COUNT][4] =
{
    ...
    [MENUITEM_BATTLE_BACKGROUND]          = {sText_Desc_NewBackgrounds_Old,       sText_Desc_NewBackgrounds_New,       sText_Desc_NewBackgrounds_Cfru},
...
void CB2_InitOptionPlusMenu(void)
{
    ...
    case 6:
        ...
        sOptions->sel_battle[MENUITEM_BATTLE_BACKGROUND]        = !gSaveBlock2Ptr->optionsNewBackgrounds ? 0 : (FlagGet(FLAG_PERFECT_BATTLE_BACKGROUNDS) ? 2 : 1);
...
static void Task_OptionMenuSave(u8 taskId)
{
    ...
    if (sOptions->sel_battle[MENUITEM_BATTLE_BACKGROUND] == 2)
        FlagSet(FLAG_PERFECT_BATTLE_BACKGROUNDS);
    else
        FlagClear(FLAG_PERFECT_BATTLE_BACKGROUNDS);
...
static void DrawChoices_New_Backgrounds(int selection, int y)
{
    // BATTLE BGROUND: Old / Modern (CFRU) / Perfect
    bool8 active = CheckConditions(MENUITEM_BATTLE_BACKGROUND);
    u8 styles[3] = {0};
    int xMid = GetMiddleX(sText_Old, sText_New, sText_Cfru);
    ...
    // three long words: the shorter "BACKGROUND" label leaves room to start at 88
    xMid = 88 + GetStringWidth(1, sText_Old, 0) + 8;
    DrawOptionMenuChoice(sText_Old, 88, y, styles[0], active);
    DrawOptionMenuChoice(sText_New, xMid, y, styles[1], active);
    DrawOptionMenuChoice(sText_Cfru, GetStringRightAlignXOffset(1, sText_Cfru, 198), y, styles[2], active);
}
