...
u16 GetCurrLocationDefaultMusic(void)
{
    ...
    // The desert's own sandstorm is set by script (dynamic weather overridden);
    // a dynamic-weather sandstorm on the grassy ends of the route keeps the
    // normal route music.
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(ROUTE111)
     && gSaveBlock1Ptr->location.mapNum == MAP_NUM(ROUTE111)
     && GetSavedWeather() == WEATHER_SANDSTORM
     && FlagGet(FLAG_DYNAMIC_WEATHER_OVERRIDDEN))
        return MUS_DESERT;
...
// v2.3: tint one sprite palette like the map (even blend-immune weather palettes).
void TintSpritePaletteLikeMap(u8 paletteNum)
{
    if (!MapHasNaturalLight(gMapHeader.mapType))
        return;
    TimeMixPalettes(
...
void CB2_ContinueSavedGame(void)
{
    ...
    DoTimeBasedEvents();
    UpdateDynamicWeatherOnContinue();
