...
// Battle idle loop (battle_weather_fx.c): end the front animation task that is
// driving this sprite, if any, so the caller can restore the sprite cleanly.
void StopMonAnimationForSprite(struct Sprite *sprite)
{
    u8 taskId;

    for (taskId = 0; taskId < NUM_TASKS; taskId++)
    {
        if (gTasks[taskId].isActive && gTasks[taskId].func == Task_HandleMonAnimation
         && (struct Sprite *)((gTasks[taskId].data[1] << 16) | ((u16)gTasks[taskId].data[2])) == sprite)
            DestroyTask(taskId);
    }
}
