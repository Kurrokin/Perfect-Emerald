...
EWRAM_DATA static u8 sStartClockWindowId = 0;   // v3.0b clock (valid only while sStartClockOn)
EWRAM_DATA static bool8 sStartClockOn = FALSE;   // EWRAM starts zeroed: never rely on initialisers
EWRAM_DATA static u8 sStartClockTimer = 0;
EWRAM_DATA static u16 sStartClockShown = 0xFFFF;         // hours * 60 + minutes on screen
...
// v3.0b: small clock at the top left, same margins as the start menu on the right
...
static void PrintStartMenuClock(bool8 force)
{
    u16 now;
    ...
    if (!sStartClockOn)
        return;
    RtcCalcLocalTime();
    now = gLocalTime.hours * 60 + gLocalTime.minutes;
    if (!force && now == sStartClockShown)
        return;
    sStartClockShown = now;
    ...
    AddTextPrinterParameterized(sStartClockWindowId, FONT_NORMAL, gStringVar4,
                                GetStringCenterAlignXOffset(FONT_NORMAL, gStringVar4, 40), 1, TEXT_SKIP_DRAW, NULL);
    CopyWindowToVram(sStartClockWindowId, COPYWIN_GFX);
}

static void ShowStartMenuClock(void)
{
    if (sStartClockOn)
        return;
    ...
    DrawStdWindowFrame(sStartClockWindowId, FALSE);   // the frame chosen in OPTION
    sStartClockTimer = 0;
    PrintStartMenuClock(TRUE);
}

static void UpdateStartMenuClock(void)
{
    if (sStartClockOn && ++sStartClockTimer >= 30)
    {
        sStartClockTimer = 0;
        PrintStartMenuClock(FALSE);
    }
}

static void RemoveStartMenuClock(void)
{
    if (!sStartClockOn)
        return;
...
static bool8 HandleStartMenuInput(void)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        RemoveStartMenuClock();   // the clock only stays while the menu itself is open
