...
static u8 GetWantedSnowFrost(void);
static void UpdateSnowFrost(void);
static EWRAM_DATA u8 sSnowFrost;
static EWRAM_DATA u8 sSnowFrostTimer;
...
static const struct WeatherCallbacks sWeatherFuncs[] =
{
    ...
    [WEATHER_MIST]               = {Mist_InitVars,          Mist_Main,          Mist_InitAll,          Mist_Finish},
};
...
void SetCurrentAndNextWeather(u8 weather)
{
    ...
    gWeatherPtr->nextWeather = weather;
    // map load: frost is either fully there (snow) or not at all
    sSnowFrost = GetWantedSnowFrost();
    sSnowFrostTimer = 0;
}
...
static void Task_WeatherMain(u8 taskId)
{
    if (gWeatherPtr->currWeather != gWeatherPtr->nextWeather)
    {
            ...
            && gWeatherPtr->palProcessingState != WEATHER_PAL_STATE_SCREEN_FADING_OUT)
        {
            ...
            // v2.9: walking into an area with other weather changes the brightness
            // three times more slowly (clear weather used to snap back at once).
            gWeatherPtr->colorMapStepDelay = (gWeatherPtr->colorMapStepDelay == 0) ? 60
                                           : min(gWeatherPtr->colorMapStepDelay * 3, 120);
...
static void ApplyColorMap_Base(u8 startPalIndex, u8 numPalettes, s8 colorMapIndex);
static void ApplyColorMapWithBlend_Base(u8 startPalIndex, u8 numPalettes, s8 colorMapIndex, u8 blendCoeff, u16 blendColor);

// ---------------------------------------------------------------------------
// Snow: while it snows, the map's tile palettes (BG palettes 0-12, not the UI)
// are frosted toward a pale blue-white, scaled by how far the snow colour map
// has faded in. Applied inside the weather colour-map pass so it survives map
// loads, day/night updates and screen fades, and fades out with the snow.
// ---------------------------------------------------------------------------
#define SNOW_TILE_TINT_COEFF 6
// v3.0: the frost has its own level that eases in and out while it snows, so it
// also shows up when the weather changes to snow from something just as dark
// (overcast, rain, ash...) - those changes never re-apply the colour map.
static EWRAM_DATA u8 sSnowFrost = 0;
static EWRAM_DATA u8 sSnowFrostTimer = 0;
#define SNOW_FROST_STEP_FRAMES 30

static void ApplySnowTileTint(u8 startPalIndex, u8 numPalettes, s8 colorMapIndex)
{
    u32 palettes = 0;
    u32 i;

    if (sSnowFrost == 0 || !MapHasNaturalLight(gMapHeader.mapType))
        return;
    for (i = startPalIndex; i < (u32)startPalIndex + numPalettes && i < 13; i++)
        palettes |= 1 << i;
    if (palettes == 0)
        return;
    BlendPalettesFine(palettes, gPlttBufferFaded, gPlttBufferFaded, sSnowFrost, RGB(27, 29, 31));
}

static u8 GetWantedSnowFrost(void)
{
    return (gWeatherPtr->currWeather == WEATHER_SNOW && MapHasNaturalLight(gMapHeader.mapType)) ? SNOW_TILE_TINT_COEFF : 0;
}

// Once per frame from the weather task: move the frost one step toward what the
// weather wants and redraw the map palettes with it.
static void UpdateSnowFrost(void)
{
    u8 wanted = GetWantedSnowFrost();

    if (sSnowFrost == wanted || gWeatherPtr->palProcessingState != WEATHER_PAL_STATE_IDLE || gPaletteFade.active)
        return;
    if (++sSnowFrostTimer < SNOW_FROST_STEP_FRAMES)
        return;
    sSnowFrostTimer = 0;
    sSnowFrost += (sSnowFrost < wanted) ? 1 : -1;
    ApplyColorMap(0, 32, gWeatherPtr->colorMapIndex);
}

static void ApplyColorMap(u8 startPalIndex, u8 numPalettes, s8 colorMapIndex)
{
    ApplyColorMap_Base(startPalIndex, numPalettes, colorMapIndex);
    ApplySnowTileTint(startPalIndex, numPalettes, colorMapIndex);
}

static void ApplyColorMapWithBlend(u8 startPalIndex, u8 numPalettes, s8 colorMapIndex, u8 blendCoeff, u16 blendColor)
{
    ApplyColorMapWithBlend_Base(startPalIndex, numPalettes, colorMapIndex, blendCoeff, blendColor);
    ApplySnowTileTint(startPalIndex, numPalettes, colorMapIndex);
}
