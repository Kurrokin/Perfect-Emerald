...
// TRUE while the player is in the Fight/Bag/Pokemon/Run menu, the move menu or
// target selection (used to loop the enemy's intro animation meanwhile).
bool8 IsPlayerInBattleMenu(void)
{
    u32 battler;

    for (battler = 0; battler < gBattlersCount; battler++)
    {
        void (*func)(void) = gBattlerControllerFuncs[battler];
        if (func == HandleInputChooseAction || func == HandleChooseActionAfterDma3
         || func == HandleInputChooseMove || func == HandleChooseMoveAfterDma3
         || func == HandleInputChooseTarget || func == HandleMoveSwitching)
            return TRUE;
    }
    return FALSE;
}
