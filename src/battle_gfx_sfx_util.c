...
void SpriteCB_EnemyShadow(struct Sprite *shadowSprite)
{
    ...
    if (gAnimScriptActive || battlerSprite->invisible)
        invisible = TRUE;
             ...
             && FlagGet(FLAG_BATTLE_SHADOWS_OFF))
        invisible = TRUE;
...
void SetBattlerShadowSpriteCallback(u8 battlerId, u16 species)
{
    ...
    // v3.0b SHADOWS = On: every opposing Pokemon gets a shadow, not only floating ones
    if (gEnemyMonElevation[species] != 0 || !FlagGet(FLAG_BATTLE_SHADOWS_OFF))
