...
static const u16 sNaturePowerMoves[] =
{
    ...
    [BATTLE_TERRAIN_ROCK_SNOW]     = MOVE_BLIZZARD,
    [BATTLE_TERRAIN_SNOW_CAVE]     = MOVE_ICE_BEAM,
    [BATTLE_TERRAIN_CAVE_WATER]    = MOVE_SURF,
    [BATTLE_TERRAIN_VOLCANO]       = MOVE_FLAMETHROWER,
    [BATTLE_TERRAIN_BLUE_BUILDING] = MOVE_SWIFT,
    [BATTLE_TERRAIN_FOREST]        = MOVE_RAZOR_LEAF,
    [BATTLE_TERRAIN_DESERT]        = MOVE_EARTHQUAKE,
    [BATTLE_TERRAIN_ASH]           = MOVE_EARTHQUAKE,
    [BATTLE_TERRAIN_CRATER]        = MOVE_HEAT_WAVE,
    [BATTLE_TERRAIN_GRAVEYARD]     = MOVE_SHADOW_BALL,
    [BATTLE_TERRAIN_TOMB_HALL]     = MOVE_SHADOW_BALL,
    [BATTLE_TERRAIN_RUINS]         = MOVE_ANCIENT_POWER,
    [BATTLE_TERRAIN_SHIP]          = MOVE_SURF,
    [BATTLE_TERRAIN_POWER_PLANT]   = MOVE_THUNDERBOLT,
    [BATTLE_TERRAIN_PATH]          = MOVE_SWIFT,   // plays like the old plain background
};
...
static const u8 sTerrainToType[] =
{
    ...
    [BATTLE_TERRAIN_ROCK_SNOW]     = TYPE_ICE,
    [BATTLE_TERRAIN_SNOW_CAVE]     = TYPE_ICE,
    ...
    [BATTLE_TERRAIN_ASH]           = TYPE_GROUND,
    [BATTLE_TERRAIN_CRATER]        = TYPE_FIRE,
    [BATTLE_TERRAIN_GRAVEYARD]     = TYPE_GHOST,
    [BATTLE_TERRAIN_TOMB_HALL]     = TYPE_GHOST,
    [BATTLE_TERRAIN_RUINS]         = TYPE_ROCK,
    [BATTLE_TERRAIN_SHIP]          = TYPE_WATER,
    [BATTLE_TERRAIN_POWER_PLANT]   = TYPE_ELECTRIC,
    [BATTLE_TERRAIN_PATH]          = TYPE_NORMAL,
};
...
{
    gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
    gCurrentMove = sNaturePowerMoves[gBattleTerrain < ARRAY_COUNT(sNaturePowerMoves) ? gBattleTerrain : BATTLE_TERRAIN_PLAIN];
    gBattlerTarget = GetMoveTarget(gCurrentMove, NO_TARGET_OVERRIDE);
    BattleScriptPush(gBattleScriptsForMoveEffects[gBattleMoves[gCurrentMove].effect]);
    gBattlescriptCurrInstr++;
}
...
static void Cmd_getsecretpowereffect(void)
{
    ...
    case BATTLE_TERRAIN_ASH:
        gBattleCommunication[MOVE_EFFECT_BYTE] = MOVE_EFFECT_ACC_MINUS_1;
        break;
    case BATTLE_TERRAIN_CAVE_WATER:
    case BATTLE_TERRAIN_SHIP:
        gBattleCommunication[MOVE_EFFECT_BYTE] = MOVE_EFFECT_ATK_MINUS_1;
        break;
    default:
        gBattleCommunication[MOVE_EFFECT_BYTE] = MOVE_EFFECT_PARALYSIS;
...
static void Cmd_settypetoterrain(void)
{
    u8 camoType = sTerrainToType[gBattleTerrain < ARRAY_COUNT(sTerrainToType) ? gBattleTerrain : BATTLE_TERRAIN_PLAIN];
    if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, camoType))
    {
        SET_BATTLER_TYPE(gBattlerAttacker, camoType);
        PREPARE_TYPE_BUFFER(gBattleTextBuff1, camoType);
